...
import { fetchListingReviews, submitReview, clearReviews } from '../redux/slices/reviewSlice';
...
const ListingDetail: React.FC = () => {
  ...
  const [isContactingSeller, setIsContactingSeller] = React.useState(false);
  ...
  // ===== Handle submit review via Redux =====
  const handleSubmitReview = useCallback(
    async (data: { rating: number; comment: string }) => {
      ...
      if (submitReview.fulfilled.match(result)) {
        ...
        showToast({
          type: 'error',
          message: (result.payload as string) || 'Gửi đánh giá thất bại. Vui lòng thử lại.',
        });
      }
    },
    [listing, dispatch, showToast]
  );
  ...
  return (
    <div className="page-enter">
      <div className="mx-auto max-w-6xl px-4 md:px-6 lg:px-8 py-6 md:py-10">
        ...
        {/* ===== Sold Banner ===== */}
        {isSold && (
          <div className="mb-6 flex items-center gap-3 rounded-2xl border border-outline-variant/20 bg-surface-container-low/50 px-5 py-4 fade-in">
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-primary-container">
              <span className="material-symbols-outlined text-[22px] text-on-primary-container">check_circle</span>
            </div>
            <div>
              <p className="text-body-md text-on-surface font-semibold">Sản phẩm đã được bán</p>
              <p className="text-label-sm text-on-surface-variant">Người bán đã đánh dấu sản phẩm này là đã bán.</p>
            </div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
          {/* Left: Image Gallery */}
          <div className={isSold ? 'relative' : ''}>
            <ImageGallery images={listing.images} alt={listing.title} />
            {isSold && (
              <div className="absolute top-4 left-4 z-10 flex items-center gap-1.5 rounded-xl bg-surface/90 px-4 py-2 shadow-lg border border-outline-variant/20 backdrop-blur-sm">
          ...
          <div className="flex flex-col">
            ...
            <div className="mt-4 flex items-center gap-3 flex-wrap">
              <p className={`text-headline-lg md:text-display-lg font-bold ${
                isSold ? 'text-on-surface-variant line-through' : 'text-primary'
              }`}>
                {formatVND(listing.price)}
              </p>
              {listing.negotiable && !isSold && (
              ...
              {isSold && (
                <span className="inline-flex items-center gap-1 rounded-lg bg-surface-variant border border-outline/10 px-3 py-1.5 text-label-sm font-semibold text-on-surface-variant">
            ...
            <div className="mt-6 flex flex-col sm:flex-row gap-3">
              {isSold ? (
                <div className="flex-1 flex items-center justify-center gap-2 rounded-2xl bg-surface-container border border-outline-variant/20 px-6 py-4">
                  <span className="material-symbols-outlined text-[20px] text-on-surface-variant">block</span>
                  <span className="text-body-md text-on-surface-variant font-medium">Sản phẩm không còn bán</span>
                </div>
              ) : (
                <>
                  {!isSeller && (
